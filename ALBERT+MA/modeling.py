...
class AlbertModel(object):
  ...
  def __init__(self,
    ...
    with tf.variable_scope(scope, default_name="bert"):
      ...
      mean_token_tensor = tf.reduce_mean(self.all_encoder_layers[-1], axis=1, keepdims=True)
      first_token_tensor = self.all_encoder_layers[0]
      last_token_tensor = self.all_encoder_layers[-1]
      concat_token_tensor = tf.concat([first_token_tensor, last_token_tensor], axis=1)

      dot_tensor = attention_layer(mean_token_tensor,
                                   concat_token_tensor,
                                   attention_mask=None,
      ...
      dot_tensor = tf.reshape(
          dot_tensor,
          [get_shape_list(dot_tensor)[0], get_shape_list(dot_tensor)[1], get_shape_list(dot_tensor)[2] * get_shape_list(dot_tensor)[3]])
      dot_tensor = tf.layers.dense(
        dot_tensor,
        config.hidden_size,
        activation=tf.tanh,
      ...
      with tf.variable_scope("pooler"):
        ...
        bilinear_tensor = add_attention_layer(mean_token_tensor,
                                                 concat_token_tensor,
                                                 attention_mask=None,
                                                 num_attention_heads=16,
                                                 query_act=None,
        ...
        bilinear_tensor = tf.layers.dense(
          bilinear_tensor,
          config.hidden_size,
          activation=tf.tanh,
          kernel_initializer=create_initializer(config.initializer_range))

        concat_tensor = add_attention_layer(mean_token_tensor,
                                               concat_token_tensor,
                                               attention_mask=None,
                                               num_attention_heads=16,
                                               query_act=None,
        ...
        concat_tensor = tf.layers.dense(
          concat_tensor,
          config.hidden_size,
          activation=tf.tanh,
          kernel_initializer=create_initializer(config.initializer_range))

        minus_tensor = add_attention_layer(mean_token_tensor,
                                             concat_token_tensor,
                                             attention_mask=None,
                                             num_attention_heads=16,
                                             query_act=None,
        ...
        minus_tensor = tf.layers.dense(
          minus_tensor,
          config.hidden_size,
          activation=tf.tanh,
          kernel_initializer=create_initializer(config.initializer_range))

        res_tensor = attention_layer(tf.concat([dot_tensor, bilinear_tensor, concat_tensor, minus_tensor], axis=1),
                                     tf.concat([dot_tensor, bilinear_tensor, concat_tensor, minus_tensor], axis=1),
                                     attention_mask=None,
                                     num_attention_heads=16,
                                     query_act=None,
                                     key_act=None,
        ...
        res_tensor = tf.reshape(
            res_tensor,
            [get_shape_list(res_tensor)[0], get_shape_list(res_tensor)[1],
             get_shape_list(res_tensor)[2] * get_shape_list(res_tensor)[3]])

        res_tensor = layer_norm(tf.reduce_mean(res_tensor, axis=1))
...
def bilinear_product_attention(q, k, v, bias, size_per_head,dropout_rate=0.0):
  """Dot-product attention.
  ...
  Returns:
    Tensor with shape [..., length_q, depth_v].
  """
  att_weight = tf.get_variable("att_new_weight",
                               shape=[size_per_head, size_per_head],
                               initializer=tf.truncated_normal_initializer(stddev=0.02))
  print('att_weight',att_weight)
  mid_attention_scors = tf.tensordot(q, att_weight, axes=1)
  attention_scores = tf.matmul(mid_attention_scors, k, transpose_b=True)

  logits = tf.multiply(attention_scores, 1.0 / math.sqrt(float(get_shape_list(q)[-1])))
  if bias is not None:
    # `attention_mask` = [B, T]
    from_shape = get_shape_list(q)
    if len(from_shape) == 4:
...
def concat_product_attention(q, k, v, bias, size_per_head,dropout_rate=0.0):
  """Dot-product attention.
  ...
  Returns:
    Tensor with shape [..., length_q, depth_v].
  """
  att_weight = tf.get_variable("att_new_weight1",
                               shape=[get_shape_list(q)[-2] + get_shape_list(k)[-2], get_shape_list(q)[-2]],
                               initializer=tf.truncated_normal_initializer(stddev=0.02))
  mid_attention_scors = tf.tensordot(tf.transpose(tf.concat([q, k], axis=2), [0, 1, 3, 2]),
                                     att_weight, axes=1)
  mid_attention_scors = tf.nn.tanh(mid_attention_scors)
  var_weight1 = tf.get_variable("var_new_weight1",
                                shape=[size_per_head, get_shape_list(k)[-2]],
                                initializer=tf.truncated_normal_initializer(stddev=0.02))
  attention_scores = tf.tensordot(tf.transpose(mid_attention_scors, [0, 1, 3, 2]), var_weight1, axes=1)
...
def minus_product_attention(q, k, v, bias, size_per_head,dropout_rate=0.0):
  """Dot-product attention.
  ...
  Returns:
    Tensor with shape [..., length_q, depth_v].
  """
  att_weight = tf.get_variable("att_new_weight2",
                               shape=[size_per_head, get_shape_list(q)[-2]],
                               initializer=tf.truncated_normal_initializer(stddev=0.02))
  mid_attention_scors = tf.tensordot(k - q, att_weight, axes=1)
  attention_scores = tf.nn.tanh(tf.transpose(mid_attention_scors, [0, 1, 3, 2]))
...
def add_attention_layer(from_tensor,
                    to_tensor,
                    attention_mask=None,
                    num_attention_heads=1,
                    query_act=None,
  ...
  if attention_mask is not None:
    ...
    # 'new_embeddings = [B, N, F, H]'
  if tag == 'bilinear':
    new_embeddings = bilinear_product_attention(q, k, v, attention_mask,size_per_head,
                                         attention_probs_dropout_prob)
  if tag == 'concat':
      new_embeddings = concat_product_attention(q, k, v, attention_mask, size_per_head,
                                                  attention_probs_dropout_prob)
  if tag == 'minus':
      new_embeddings = minus_product_attention(q, k, v, attention_mask, size_per_head,
                                                  attention_probs_dropout_prob)
  context_layer=tf.transpose(new_embeddings, [0, 2, 1, 3])
